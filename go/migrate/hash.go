...
func repoCallsForOperation(table Table, operation string, tableMap map[string]Table, resolved map[funcKey]resolvedFunc) ([]repoCallRef, error) {
	...
	case "list":
		// List materializes each matching row via get (including nested children).
		if err := add(table.Schema, table.Name, "get"); err != nil {
			return nil, err
		}
	}
...
func repoOperations() []string {
	return []string{"insert", "update", "upsert", "delete", "get", "list"}
}
